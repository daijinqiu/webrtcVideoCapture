...
public class EglRenderer implements VideoSink {
    ...
    private static class HandlerWithExceptionCallback extends Handler {
        ...
        public void dispatchMessage(Message msg) {
            try {
                ...
                Log.e(TAG, "Exception on EglRenderer thread", e);
    ...
    private final GlTextureFrameBuffer bitmapTextureFramebuffer = new GlTextureFrameBuffer(GLES20.GL_RGBA);
    ...
    public void init(final EglBase.Context sharedContext, final int[] configAttributes, RendererCommon.GlDrawer drawer, boolean usePresentationTimeStamp) {
        synchronized (handlerLock) {
            ...
            renderThreadHandler.postDelayed(logStatisticsRunnable, TimeUnit.SECONDS.toMillis(LOG_INTERVAL_SEC));
        }
    }
    ...
    public void onFrame(VideoFrame frame) {
        Log.d(TAG, "onFrame framesReceived: " + framesReceived);
        synchronized (statisticsLock) {
            ++framesReceived;
        }
    ...
    private void renderFrameOnRenderThread() {
        logD("renderFrameOnRenderThread enter");
        ...
        drawMatrix.preTranslate(-0.5f, -0.5f);
        logD("renderFrameOnRenderThread shouldRenderFrame = " + shouldRenderFrame);
        try {
